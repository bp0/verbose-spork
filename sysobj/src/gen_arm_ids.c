...
static void gen_arm_ids_cache_item(arm_id *pid) {
    ...
    if (pid->implementer) {
        sprintf(buff, ":/cpu/cpuinfo/arm.ids/%02x", pid->implementer);
    ...
    if (pid->part) {
        sprintf(buff, ":/cpu/cpuinfo/arm.ids/%02x/%03x", pid->implementer, pid->part);
        sysobj_virt_add_simple(buff, NULL, "*", VSO_TYPE_DIR );
        if (pid->part_str)
...
static gchar *gen_arm_ids_lookup_value(const gchar *path) {
    ...
    int mc = sscanf(path, ":/cpu/cpuinfo/arm.ids/%02x/%03x", &pid->implementer, &pid->part);
    gchar *verify = NULL;
    switch(mc) {
        case 2:
            if (!verify)
            verify = (name_id == -1)
                ? g_strdup_printf(":/cpu/cpuinfo/arm.ids/%02x/%03x/%s", pid->implementer, pid->part, name)
                : g_strdup_printf(":/cpu/cpuinfo/arm.ids/%02x/%03x", pid->implementer, pid->part);
        case 1:
            if (!verify)
            verify = (name_id == -1)
                ? g_strdup_printf(":/cpu/cpuinfo/arm.ids/%02x/%s", pid->implementer, name)
                : g_strdup_printf(":/cpu/cpuinfo/arm.ids/%02x", pid->implementer);
...
void gen_arm_ids() {
    ...
    vo->path = g_strdup(":/cpu/cpuinfo/arm.ids");
    vo->str = g_strdup("*");
