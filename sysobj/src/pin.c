...
void pins_free(pin_list *pl) {
    if (pl) {
        ...
        g_slist_free_full(pl->list, pin_free);
        g_slist_free_full(pl->groups, g_free);
        g_free(pl);
    }
}
 ...
 * path=options
 *   or
 * @path=all children of this path, using the same options
 */
...
pin_list *pins_new_from_kv(const gchar *kv_data_in) {
    ...
    for (i = 0; groups[i]; i++) {
        if (!!strcmp(groups[i], before_first_group)) {
            group_id++;
            pl->groups = g_slist_append(pl->groups, groups[i]);
        }
        keys = g_key_file_get_keys(key_file, groups[i], NULL, NULL);
        for (j = 0; keys[j]; j++) {
            gchar *options = g_key_file_get_value(key_file, groups[i], keys[j], NULL);
            gchar *path = keys[j];
            if (*path == '@') {
                sysobj *obj = sysobj_new_from_fn(path+1, NULL);
                if (obj && obj->exists) {
                    GSList *childs = sysobj_children(obj, NULL, NULL, FALSE);
                    for (l = childs; l; l = l->next) {
                        int pi = pins_add_from_fn(pl, obj->path, (gchar*)l->data);
                        pin *p = pins_get_nth(pl, pi);
                        if (p) p->group = group_id;
                    }
                    g_slist_free_full(childs, g_free);
                }
                sysobj_free(obj);
            } else {
                int pi = pins_add_from_fn(pl, path, NULL);
                pin *p = pins_get_nth(pl, pi);
                if (p) p->group = group_id;
            }
            g_free(options);
